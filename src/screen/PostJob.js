...
hideLoader()
    if (getLatLongApiData.type === GETLATLONG) {
      if (
        ...
        setLatitude(getLatLongApiData?.value?.Items[0]?.Latitude)
        setLogitude(getLatLongApiData?.value?.Items[0]?.Longitude)
  ...
  console.log("postCode>>",postCode);
    ...
    console.log("dataPostCode>>>   dataPostCode.data.Items",  dataPostCode.data.Items);
...
console.log("getPostCodeData>>>",JSON.stringify(getPostCodeData));
...
let flatNo3 = getPostCodeData.data.Items[0].BuildingName?getPostCodeData.data.Items[0].BuildingName:""
  ...
  useEffect(() => {
    ...
    if (postjobShiftValidation.type === POSTJOBSHIFTVALIDATION) {
      if (
        ...
        setTimeout(() => {
          // snackbarError(error.response.data.Message);
          snackbarSuccess(postjobShiftValidation.value.message);
        }, 100);
        //  setUpadteDate('')
        //  setSendApiDate([])
        setType(3);
  ...
  useEffect(() => {
    ...
    if (getPostJobDropDownLists.type === POST_JOB_DROPDOWN_DATA) {
      if (
        ...
        getPostJobDropDownLists.value.extraSupports.map((v, i) => {
          let temp = {
            id: v.id,
            title: v.extra_support,
            isSelected: isSelected,
          };
  ...
  const chooseStartTimeSlots = (item, index) => {
    console.log("item slect start time",item);
    console.log("shifts?.title.>>>>>",shifts?.title);
    setTime(item.title);
...
<Image
        ...
        <View style={{flex:1}} >
          ...
          <View style={{ marginTop: mvs(20) }}>
            <Text style={styles.textTitle}>
            Gender Preference <Text style={{ color: colors.starcolor }}>*</Text>
            </Text>
            <MyDropDown
              // refs={refSubject}
          ...
          <View style={{ marginTop: mvs(20),marginBottom:mvs(60) }}>
            <Text style={styles.textTitle}>
              {' '}
        ...
        <ScrollView
          ...
          <View style={{ marginTop: mvs(10) }}>
            <Text style={styles.textTitle}>
              Professional Experience{' '}<Text style={{ color: colors.starcolor }}>*</Text>
...
<Text style={{fontSize:12,fontFamily:fonts.quicksandMedium,color:colors.black,padding:10}}>Please enter any additional requirements here</Text>
</View>
  }
>
<View style={{ flexDirection: 'row' }}>
